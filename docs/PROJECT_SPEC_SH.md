...
- `/etc/profile` gets staging tokens rewritten to production and a fixed proxy set (`http://172.16.162.6:3128/`) appended whenever those exports are missing. It also adds `no_proxy` / `NO_PROXY` / `NO_PROXT` with `127.0.0.1, localhost, 172.16.0.0/16` (hosts matching `*-01s` / `*-01p` gain `<prefix>-01h` and `<prefix>-02h` entries) when absent.
