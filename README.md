...
- Treasure Data (td-agent) リポジトリ生成 📦:
  ...
  - `yum update td-agent --disablerepo=* --enablerepo=treasuredata` を実行し、ログを `/tmp/<user>/find_and_extract/td-agent-update.log` に保存。未インストール時は `yum install td-agent --disablerepo=* --enablerepo=treasuredata` を実行し、結果を `/tmp/<user>/find_and_extract/td-agent-install.log` に記録
- 編集途中ファイルを誤変換しない配慮 🙌:
...
- `remi-safe`, `remi-php*`, `zabbix`, `zabbix-non-supported` のリポジトリを自動無効化し、`yum update td-agent --disablerepo=* --enablerepo=treasuredata`（未導入の場合は `yum install` を追加実行）を実施
