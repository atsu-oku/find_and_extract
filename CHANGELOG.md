- v3.5.1.0 (2025-10-30): ログパスに含まれる staging/production を新基盤判定へ再分類し、環境メッセージを拡充。
- v3.5.0.0 (2025-10-30): バックアップファイルを `*_YYYYMMDD_HHMM.bak` 形式へ統一し、混在定義ログの出力とバックアップファイル検出の拡張、ループバック行を含む newstaging/newstg 置換を強化。

# find_and_extract.sh Version History



- v3.4.4.3 (2025-10-30): `hostname-ap01s` のようなホスト名でも `01s` → `01p` に変換されるよう `0Ns` 置換を拡張。

- v3.4.4.2 (2025-10-30): ループバック行に対する `newstaging` / `newstg` の保護を解除し、その他のコメント除外ルールのみ維持。

- v3.4.4.1 (2025-10-30): ループバック行 (`127.0.0.1` / `::1`) を `newstaging` / `newstg` 置換対象から除外。

- v3.4.4.0 (2025-10-30): コメント以外の `newstaging` / `newstg` を一括で `newproduction` / `newprd` に置換する変換ルールを追加。

- v3.4.3.0 (2025-10-29): コメント行（`#` 以降）を変換対象から除外し、`01s`〜`09s` のみを `01p`〜`09p` に置換するようルールを限定。

- v3.4.2.0 (2025-10-29): スクリプト本体とリポジトリ内のバージョン番号を 3.4.2.0 に更新。

- v3.4.0.0: 変換ログの記録とロールバック機能を実装し、`rollback` サブコマンドと必須ディレクトリ/ファイルチェックを追加。



- v3.3.0.0: transform サブコマンドを追加し、ドライランをデフォルト化。実変更時には確認プロンプトとバックアップ作成を実施。



- v3.2.4.0 (2025-08-01): 現行基盤ホスト名の正規表現を再度修正し、'dmp', 'ltt', 'biz' プレフィックスを持つサーバーの検出漏れを解消。



- v3.2.3.0: 現行基盤ホスト名の正規表現をさらに修正し、'dmp', 'ltt', 'biz' プレフィックスを持つサーバーの検出漏れを解消。



- v3.2.2.0: 現行基盤ホスト名の正規表現を修正し、'dmp', 'ltt', 'biz' プレフィックスを持つサーバーの検出漏れを解消。



- v3.2.1.0: 現行基盤のIPアドレス検索条件からAWS関連の条件を分離。



- v3.2.0.0: 現行基盤のIPアドレスおよびホスト名の正規表現を修正し、検出漏れを解消。



- v3.1.9.0: 現行基盤ホスト名の正規表現を修正し、'dmp', 'ltt', 'biz' プレフィックスの検出漏れを解消。



- v3.1.8.0: スクリプト実行時に表示されるヘッダーのフォーマット不具合を修正。



- v3.1.7.0: 新基盤ホスト名の正規表現を修正し、'dlagency' と 'agency' プレフィックスの検出漏れを解消。



- v3.1.6.0: 現行基盤ホスト名の正規表現を修正し、'dmp' プレフィックスの検出漏れを解消。



- v3.1.5.0: 現行基盤のIPアドレス検索条件を追加。



- v3.1.4.0: 実行開始時に区切り線が複数表示される不具合を修正。



- v3.1.3.0: 環境文字列の検索条件を現行・新基盤に正しく振り分け。



- v3.1.2.0: LPIC Level1 技術者向けに、より詳細なコメントを追記。



- v3.1.1.0: 指示に基づき、詳細な改変履歴をヘッダーに記載。



- v3.1.0.0: LPIC Level2 技術者向けに詳細な日本語コメントを付加。



- v3.0.0.0: 標準出力サマリーの詳細表示部分の改行不具合を修正。



- v2.9.9.0: 標準出力サマリーの詳細表示部分に改行を追加。



- v2.9.8.0: 新基盤ホスト名の Staging/Production 判定ロジックを堅牢な方式に修正。



- v2.9.7.0: 新基盤ホスト名の正規表現を修正し、数字の桁数を可変長に変更。



- v2.9.6.0: 現行基盤ホスト名の正規表現を修正し、'dmp' プレフィックスの検出漏れを解消。



- v2.9.5.0: ホスト名の Staging/Production 判定ロジックを修正し、誤判定を防止。



- v2.9.4.0: 詳細な改変履歴をコード内に記載。



- v2.9.3.0: 「新基盤」と「現行基盤」の定義が逆だった問題を修正。



- v2.9.2.0: `is_backup_file_name` 関数の未定義エラーを修正。



- v2.9.1.0: SELinux 関連パスと「#」を含むファイル名を除外。



- v2.9.0.0: ログの趣旨に合わせて「メインログ」を「現行基盤定義」等に名称変更（後に誤りと判明）。



- v2.8.9.0: ループ内での `local` 変数宣言エラーを修正し、「その他条件」を詳細化。



- v2.8.8.0: 現行基盤の旧ホスト名リストを正規表現パターン化。



- v2.8.7.0: 未知のプレフィックスを持つ新基盤ホスト名を検出する汎用的な正規表現を追加。



- v2.8.6.0: 標準出力サマリーを Staging/Production に分けて表示するよう修正。



- v2.8.5.0: 新基盤ホスト名定義の末尾 s/p を両方許容するよう変更。



- v2.8.4.0: 新基盤ホスト名リストの固定数値を正規表現に汎化。



- v2.8.3.0: 新基盤ホスト名リストを網羅する正規表現を定義。



- v2.8.2.0: 不正な IP アドレス形式の検出機能を追加。



- v2.8.1.0: 複合検索条件を個別の条件に分解。



- v2.8.0.0: 検索条件を連想配列にリファクタリング。



- v2.7.0.1: 提供された初期バージョン。

